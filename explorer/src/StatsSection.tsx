...
const StatsSection: React.FC<StatsSectionProps> = ({
  ...
  const notarizationTimes = views
    .filter(
      (view) => view.status === "notarized" || view.status === "finalized"
    )
    .map((view) => {
      if (
        view.actualNotarizationLatency !== undefined &&
        view.actualNotarizationLatency > 0
      ) {
  ...
  const finalizationTimes = views
    ...
    .map((view) => {
      if (
        view.actualFinalizationLatency !== undefined &&
        view.actualFinalizationLatency > 0
      ) {
  ...
  const medianTimeToLock =
    sortedNotarizationTimes.length > 0
      ? sortedNotarizationTimes.length % 2 === 1
        ? sortedNotarizationTimes[
            Math.floor(sortedNotarizationTimes.length / 2)
          ]
        : Math.round(
  ...
  const medianTimeToFinalize =
    sortedFinalizationTimes.length > 0
      ? sortedFinalizationTimes.length % 2 === 1
        ? sortedFinalizationTimes[
            Math.floor(sortedFinalizationTimes.length / 2)
          ]
        : Math.round(
  ...
  return (
    <div className="stats-card">
          ...
          <div className="browser-metrics-container">
            ...
            <Tooltip content={tooltips.timeToFinalize}>
              <div className="metric-container">
                ...
                <div className="stat-value">
                  {medianTimeToFinalize > 0
                    ? `${medianTimeToFinalize}ms`
                    : "N/A"}
                </div>
              </div>
            </Tooltip>
      ...
      <div className="stats-disclaimer">
        All latency measurements made by your browser are only performed after
        verifying the integrity of incoming artifacts with the network key.

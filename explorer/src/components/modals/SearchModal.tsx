...
const SearchModal: React.FC<SearchModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const initWasm = async () => {
      try {
        ...
        setError(
          "Failed to initialize search functionality. Please try again later."
        );
      }
    };
  ...
  const parseQuery = (
    ...
    if (
      type === "block" &&
      (query.startsWith("0x") || /^[0-9a-fA-F]{64}$/.test(query))
    ) {
  ...
  const fetchData = async () => {
    ...
    if (parsedQuery === null) {
      setError(
        `Invalid query: "${searchQuery}". Please enter a valid number, range (e.g., "10..20"), or "latest".`
      );
      setIsLoading(false);
      return;
    }
  ...
  const fetchSingleItem = async (
    ...
    if (!wasmInitialized) {
      throw new Error(
        "Search functionality is still initializing. Please try again in a moment."
      );
    }
    ...
    switch (searchType) {
      case "block":
        endpoint = `/block/${
          typeof query === "number" ? numberToU64Hex(query) : query
        }`;
        break;
      case "notarization":
        endpoint = `/notarization/${
          typeof query === "number" ? numberToU64Hex(query) : query
        }`;
        break;
      case "finalization":
    ...
    try {
      ...
      try {
        if (searchType === "seed") {
          const result = {
            view: 0,
            signature: new Uint8Array([0, 0, 0, 0, 0, 0]),
          };
          ...
          if (query === "latest" || typeof query === "number") {
            ...
            if (!result)
              throw new Error("Failed to parse block data by digest");
            return result;
          }
        }
      } catch (parseError) {
      ...
      console.warn(
        `Unexpected data format for ${searchType}, returning raw data`
      );
      return data as any;
    } catch (error) {
  ...
  const renderSearchResult = (item: SearchResultWithLatency, index: number) => {
    ...
    if ("view" in result && "signature" in result && !("proof" in result)) {
      ...
      resultType =
        lastSearchType === "finalization" || lastSearchType === "block"
          ? "Finalization"
          : "Notarization";
      ...
      <div className="search-modal">
        ...
        {showHelp && (
          <div className="search-help-panel">
            ...
            <ul>
              <li>
                <strong>Block</strong>: by height (number), digest (hex), or
                "latest"
              </li>
              <li>
                <strong>Notarization</strong>: by view number or "latest"
              </li>
              <li>
                <strong>Finalization</strong>: by view number or "latest"
              </li>
              <li>
                <strong>Seed</strong>: by view number or "latest"
              </li>
            </ul>
            <p>
              You can also search for ranges (e.g., "10..20") to get multiple
              results.
            </p>
          </div>
        )}
            ...
            {results.length > 0 ? (
              ...
              !error && (
                <div className="search-no-results">
                  {isLoading
                    ? "Searching..."
                    : "No results to display. Try a different search."}
                </div>
              )
            )}
          </div>
        ...
        <div className="search-modal-footer">
          <button className="search-button cancel" onClick={onClose}>
            Close
          </button>
        </div>
      </div>

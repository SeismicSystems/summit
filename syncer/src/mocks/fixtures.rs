...
use commonware_cryptography::{
    ...
    bls12381::{
        dkg,
        primitives::{group, ops, sharing::Mode, variant::Variant},
    },
    ed25519,
};
use commonware_math::algebra::Random;
use commonware_utils::NZU32;
use commonware_utils::{TryCollect, ordered::BiMap, quorum};
    ...
    R: RngCore + CryptoRng,
{
    ...
    let bls_public: Vec<_> = bls_privates
        .iter()
        .map(|sk| ops::compute_public::<V>(sk))
        .collect();
    ...
    R: RngCore + CryptoRng,
{
    ...
    let participants = ed25519_participants(rng, n).into_keys();
    let n_nz = std::num::NonZeroU32::new(n).unwrap();
    let (polynomial, shares) = dkg::deal_anonymous::<V>(&mut *rng, Mode::default(), n_nz);

    let schemes: Vec<_> = shares
        .into_iter()
        .map(|share| {
            bls12381_threshold::Scheme::signer(participants.clone(), polynomial.clone(), share)
                .unwrap()
        })
        .collect();

...
/// to the syncer [Actor](crate::actor::Actor).
...
/// A handler that forwards requests from the resolver to the syncer actor.
///
...
/// metadata attached to that lookup so the syncer can decide how to process
/// the response after validating it against the key. It is not part of peer
...
///
/// The `Finalized` height is a raw `u64` on the wire (fixed-width encoding)
/// rather than a varint-encoded [`Height`].
#[derive(Clone, Copy)]
...
/// A valid syncer backfill fetch request.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum RequestKind<D: Digest> {
...
/// A syncer backfill fetch with a request and local processing annotation that match.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
pub(crate) fn above_height_floor<D: Digest>(
    ...
    move |request, annotation| match (request, annotation) {
        (Key::Finalized { height: requested }, _) => *requested > height.get(),
        (
            Key::Block(_),
...
mod tests {
    ...
    fn test_subject_finalized_encoding() {
        let request = Key::<D>::Finalized { height: 12345u64 };

        // Test encoding: fixed-width u64 height on the wire.
        let encoded = request.encode();
        assert_eq!(encoded.len(), 9); // 1 byte for enum variant + 8 bytes for height
        ...
        let decoded = Key::<D>::read(&mut buf).unwrap();
        assert_eq!(request, decoded);
        assert_eq!(decoded, Key::Finalized { height: 12345 });
    }
    ...
    fn test_subject_hash() {
        ...
        let r1 = Key::<D>::Finalized { height: 100 };
        let r2 = Key::<D>::Finalized { height: 100 };
        let r3 = Key::<D>::Finalized { height: 200 };
    ...
    fn test_height_floor_predicate() {
        ...
        let higher_finalized = Key::<D>::Finalized { height: 200 };
        ...
        let same_height = Key::<D>::Finalized { height: 100 };
        assert!(!predicate(
            &same_height,
            &Annotation::Finalized(Finalized::ByHeight {
                height: Height::new(100),
    ...
    fn test_round_floor_predicate() {
        ...
        let finalized = Key::<D>::Finalized { height: 100 };
    ...
    fn test_encode_size() {
        ...
        let r2 = Key::<D>::Finalized { height: u64::MAX };
    ...
    fn test_request_ord_same_variant() {
        ...
        let fin1 = Key::<D>::Finalized { height: 100 };
        let fin2 = Key::<D>::Finalized { height: 200 };
        let fin3 = Key::<D>::Finalized { height: 200 };
    ...
    fn test_request_ord_sorting() {
        ...
        assert_eq!(sorted[3], Key::<D>::Finalized { height: 100 });
        assert_eq!(sorted[4], Key::<D>::Finalized { height: 200 });
    ...
    fn test_request_ord_edge_cases() {
        ...
        let min_finalized = Key::<D>::Finalized { height: 0 };
        let max_finalized = Key::<D>::Finalized { height: u64::MAX };

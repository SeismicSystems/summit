...
fn extract_endpoint(key: &str, line: &str) -> Option<SocketAddr> {
...
impl std::fmt::Display for NodeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            NodeError::NoStdout => write!(f, "No stdout available"),
            NodeError::SpawnError(e) => write!(f, "Spawn error: {}", e),
            NodeError::CreateDirError(e) => write!(f, "Create dir error: {}", e),
            NodeError::Timeout => write!(f, "Timeout"),
            NodeError::ReadLineError(e) => write!(f, "Read line error: {}", e),
            NodeError::Fatal(s) => write!(f, "Fatal error: {}", s),
        }
    }
}

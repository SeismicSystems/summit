...
impl Reth {
    ...
    /// Skips the P2P readiness check without adding `--disable-discovery` flags.
    ///
    /// Use this when managing discovery flags manually (e.g. discv5-only mode)
    /// and you don't need the builder to wait for the "Updated local ENR" log line.
    pub const fn skip_p2p_check(mut self) -> Self {
        self.skip_p2p_check = true;
        self
    }
    ...
    pub fn try_spawn(self) -> Result<RethInstance, NodeError> {
        ...
        let mut p2p_started = !self.discovery_enabled || self.skip_p2p_check;
